...
1. Клонируйте репозиторий ```git clone https://github.com/MurtazinaAlina/graduate_work.git```
...
7. Настроены __debug_toolbar__ и __django-silk__ для аналитики и оптимизации
запросов. Silk доступен по url http://127.0.0.1:80/silk/
...
_ФИО получателя - при желании._ Если получатель передан в аргументе явно,
будет назначен он, если же получатель не передан - им будет по умолчанию назначен
пользователь, разместивший заказ.

    {
    "contact": 6,
    "delivery_date": "2023-12-31",
    "delivery_time": "afternoon_15_18",
    "recipient_full_name": "Фамилия Имя Отчество" - ПО ЖЕЛАНИЮ
    }
